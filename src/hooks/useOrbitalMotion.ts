 ...
 *
 * Under reduced motion the bodies hold their starting angle: the layout is
 * identical, it simply stops moving.
 */
export function useOrbitalMotion(orbit: OrbitParams, enabled = true) {
  ...
  useFrame(({ clock }) => {
    ...
    const angle = moving ? orbit.phase + clock.elapsedTime * orbit.speed : orbit.phase
    group.position.set(
